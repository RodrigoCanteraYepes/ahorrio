...
export function renderEditView(root) {
  root.innerHTML = "";

  const route = currentRoute();
  const txId = route.params.get("id");
  if (!txId) { navigate("dashboard"); return; }

  const allTxs = loadTransactions();
  const tx = allTxs.find((t) => t.id === txId);
  if (!tx) { navigate("dashboard"); return; }
  ...
  const screen = h("section", { className: "add-view", "aria-labelledby": "edit-title" });

  // header
  const back = h("button", { className: "add-view__back", type: "button", "aria-label": "Volver al inicio" }, "\u2190 Atr\u00e1s");
  back.addEventListener("click", () => {
    if (dirty && !window.confirm("Ten\u00e9s cambios sin guardar. \u00bfSalir de todas formas?")) return;
    navigate("dashboard");
  });
  ...
  // form
  const form = h("form", { className: "add-view__form", noValidate: true, "aria-label": "Formulario de edici\u00f3n" });
  ...
  for (const opt of /** @type {const} */ (["Gasto", "Ingreso"])) {
    ...
    b.addEventListener("click", () => { setTipo(opt); markDirty(); });
    seg.appendChild(b);
  ...
  // Importe
  const importeInput = h("input", { className: "field__input", type: "text", id: "edit-importe", inputMode: "decimal", autocomplete: "off", placeholder: "0,00", value: formatEUR(tx.amountCents).replace(" €", "") });
  ...
  // Categor\u00eda
  const catSelect = h("select", { className: "field__input field__input--select", id: "edit-categoria" });
  const catError = errPara();
  form.appendChild(buildField("Categor\u00eda", catSelect, catError));

  // Fecha
  const fechaInput = h("input", { className: "field__input", type: "date", id: "edit-fecha", value: fecha });
  fechaInput.addEventListener("change", () => { fecha = /^\d{4}-\d{2}-\d{2}$/.test(fechaInput.value) ? fechaInput.value : tx.dateISO; markDirty(); });
  form.appendChild(buildField("Fecha", fechaInput));

  // Descripci\u00f3n
  const descInput = h("input", { className: "field__input", type: "text", id: "edit-desc", maxLength: 120, placeholder: "Descripci\u00f3n (opcional)", value: description });
  descInput.addEventListener("input", () => { description = descInput.value; markDirty(); });
  ...
  // wiring
  catSelect.addEventListener("change", () => { catError.hidden = true; updateSaveEnabled(); markDirty(); });
  importeInput.addEventListener("input", () => { importeError.hidden = true; updateSaveEnabled(); markDirty(); });
  ...
  function populateCategorias() {
    ...
    // Pre-select the transaction's category
