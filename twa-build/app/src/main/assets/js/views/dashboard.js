...
function createMenuPanel(root) {
  ...
  // --- Section: Apariencia ---
  const themeSection = el("div", "menu-section");
  themeSection.appendChild(el("h3", "menu-section__title", "Apariencia"));
  const themeToggle = el("div", "theme-toggle");
  const themeLabel = el("span", "theme-toggle__label", isDarkActive() ? "🌙 Modo oscuro" : "☀️ Modo claro");
  const themeSwitch = el("button", `theme-toggle__switch ${isDarkActive() ? "theme-toggle__switch--active" : ""}`);
  themeSwitch.type = "button";
  themeSwitch.setAttribute("aria-label", "Cambiar tema");
  themeSwitch.addEventListener("click", () => {
    const next = isDarkActive() ? "light" : "dark";
    applyTheme(next);
    themeLabel.textContent = next === "dark" ? "🌙 Modo oscuro" : "☀️ Modo claro";
    themeSwitch.classList.toggle("theme-toggle__switch--active", next === "dark");
  });
